...
{int} {
		long int ret = strtol(yytext, NULL, 10);

		if (ret <= TIGER_INT_MAX ){
		/* reject leading zeros !*/
		/*	int n = strspn(yytext, "0") ;
			if (n != 0 && yytext[n] != '\0'){
					char* tmp = malloc(sizeof(char) * 256);
					strcpy(tmp, (char*) yytext+n);
					yytext = tmp;
					ret = strtol(yytext, NULL, 10);
			}
		*/
			return yy::tiger_parser::make_INT(ret, loc);
		}
		else
			utils::error(loc, "invalid integer");
}
